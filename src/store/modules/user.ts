...
const useUserStoreFunc = defineStore("user", {
  state: () => {
    return {
      currentUser: storageLocal.getItem<UserDto>(STORAGE_KEY_USER_INFO),
  ...
  actions: {
    ...
    updateUserInfo(nickName: string, avator: string) {
      ...
      this.currentUser!.avatar = avator;
    },
    ...
    getAuthorization() {
      return storageLocal.getItem<string>(STORAGE_KEY_AUTHORIZATION);
    },
  },
});
