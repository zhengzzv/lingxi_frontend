<template>
  ...
  <div class="h-screen bg-white bg-no-repeat bg-cover flex justify-center items-center"
    ...
    <div class="bg-white w-96 shadow-md rounded px-8">
      <div class="bg-contain bg-no-repeat bg-center h-32" style="background-image: url(/lingxi.svg);">
      </div>
      <el-input v-model="data.username" placeholder="请输入用户名" clearable autofocus class="my-2" />
      <el-input v-model="data.password" type="password" placeholder="请输入密码" show-password clearable class="my-2"
        @keyup.enter="login" />
      <el-input clearable :input-style="{ 'user-select': 'none' }" v-model="data.inputVerifyCode" class="my-2"
        placeholder="验证码" @keyup.enter="login">
...
async function login() {
  ...
  // 测试jwt token链路有效
  const userInfo = (await api.UserAPi.currentUser()).data
